...
export function DashboardPage() {
  ...
  useEffect(() => {
    setLoading(true);
    gqlQuery<{ projects: Project[] }>("query { projects { id code name progress type } }")
      .then(async (res) => {
        ...
        await Promise.all(
          projectList.map(async (project) => {
            const taskRes = await gqlQuery<{ tasks: Task[] }>("query ($projectId: ID!) { tasks(projectId: $projectId) { id title progress riskLevel isOverdue status { name code isDone } } }", { projectId: project.id });
  ...
  return (
    <div style={{ padding: 16 }}>
      <Spin spinning={loading}>
        <Row gutter={[16, 16]}>
          <Col span={8}><Card><Statistic title="Проекты" value={projects.length} /></Card></Col>
          <Col span={8}><Card><Statistic title="Задачи" value={totals.tasks} /></Card></Col>
          <Col span={8}><Card><Statistic title="Выполнено" value={totals.done} /></Card></Col>
          <Col span={8}><Card><Statistic title="Средний прогресс" value={totals.avgProgress} suffix="%" /></Card></Col>
          <Col span={8}><Card><Statistic title="Просрочено" value={totals.overdue} /></Card></Col>
          <Col span={8}><Card><Statistic title="Высокий риск" value={totals.highRisk} /></Card></Col>
        </Row>
